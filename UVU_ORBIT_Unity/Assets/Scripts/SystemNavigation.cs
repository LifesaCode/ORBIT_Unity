...
public class SystemNavigation : MonoBehaviour
{
    ...
    SpriteRenderer[] downstates;
    Image[] buttons;
    bool systemsOn = false;

    private void Start()
    {
        buttons = Buttons.GetComponentsInChildren<Image>();
        downstates = Downstates.GetComponentsInChildren<SpriteRenderer>();
        Debug.Log("Downstates length = " + downstates.Length);
        GetCabin();
    }
